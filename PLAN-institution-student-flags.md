...
- **`frontend/src/features/principal/students/AllStudents.jsx`** (route `/app/all-students`, a master-detail layout, not a table): kept its existing three-dot `Dropdown` in the profile header as-is (you asked not to copy `StudentList.jsx`'s inline-icons/clickable-tag style here) — Edit Profile / Add Document / Reset Credential / **Mark as Placed** (new item, same placement modal as §6) / Deactivate·Activate. The only behavior change is the "Deactivate" menu item: clicking it now opens the shared `DeactivationConfirmModal` (reason + remarks) instead of a plain `Modal.confirm`; "Activate" still uses the simple `Modal.confirm` since no reason is needed. The Status tag next to the student's name was left as plain, non-clickable display, matching its original behavior.
