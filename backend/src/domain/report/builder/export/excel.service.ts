...
export class ExcelService {
  ...
  async generateExcel(config: ExportConfig): Promise<Buffer> {
    ...
    // Merge cells vertically for grouped columns (e.g. company name spanning multiple institute rows)
    if (config.mergeColumns && config.mergeColumns.length > 0) {
      const headerRowNumber = config.metadata ? 5 : 4; // account for title + metadata + empty row + header
      const dataStartRow = headerRowNumber + 1;

      for (const mergeField of config.mergeColumns) {
        const colIndex = config.columns.findIndex(c => c.field === mergeField);
        if (colIndex === -1) continue;

        const excelCol = colIndex + 1;
        let rangeStart = dataStartRow;

        for (let r = dataStartRow + 1; r <= dataStartRow + config.data.length; r++) {
          const currentVal = worksheet.getCell(r, excelCol).value;
          const startVal = worksheet.getCell(rangeStart, excelCol).value;

          if (r === dataStartRow + config.data.length || currentVal !== startVal) {
            // Merge the range if it spans more than one row
            if (r - 1 > rangeStart) {
              worksheet.mergeCells(rangeStart, excelCol, r - 1, excelCol);
              const mergedCell = worksheet.getCell(rangeStart, excelCol);
