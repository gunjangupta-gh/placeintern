...
const TestResponsesPage = () => {
  ...
  const fetchSummary = useCallback(async (isRefresh = false) => {
    try {
      ...
      const response = await trainingPrincipalService.getTestSummary();
  ...
  const fetchTestResponses = async (trainingId) => {
    try {
      ...
      const [preTestRes, postTestRes] = await Promise.all([
        trainingPrincipalService.getPreTestResponses(trainingId),
        trainingPrincipalService.getPostTestResponses(trainingId),
      ]);
    ...
    <div className="p-4 training-ui">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/app/training')}>
            Back
          </Button>
          <Button
                  ...
                  return (
                    <div key={row.id} className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <div>
                          <Text className="text-xs text-slate-500 block">Q{index + 1}</Text>
                          <Text className="text-sm text-slate-800 block">{row.label}</Text>
                        </div>
                        {correctness !== null && (
                          <Tag color={correctness ? 'green' : 'red'} className="m-0 text-xs">
                      ...
                      <div className="mt-1">
                        <Text className="text-xs text-slate-500 block">Answer</Text>
                        <Text className="text-sm text-slate-800">{renderAnswerValue(row.answer)}</Text>
                      </div>

                      {row.correctAnswer !== undefined && row.correctAnswer !== null && (
                        <div className="mt-1.5">
                          <Text className="text-xs text-slate-500 block">Expected</Text>
                          <Text className="text-sm text-slate-700">{renderAnswerValue(row.correctAnswer)}</Text>
                        </div>
                      )}
                    </div>
                  );
